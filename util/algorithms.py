...
class CounterfactualRegretMinimizationBase:
    ...
    def _cfr_utility_recursive(self, state, reach_a, reach_b):
        ...
        # we are computing regrets for both players simultaneously,
        # therefore we need to relate reach,reach_opponent to the player acting
    ...
    def __value_of_the_game_state_recursive(self, node):
        ...
        for action in node.actions:
            value += self.nash_equilibrium[node.inf_set()][action] * \
                     self.__value_of_the_game_state_recursive(node.play(action))
        return value
